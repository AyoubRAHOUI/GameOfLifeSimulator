...
public class Editor {
    ...
    public void handleSimulatorEven(SimulatorEvent event) {
    ...
    private void boardPressed(CellPosition cursorPosition) {
        ...
        if (drawingEnabled) {
            ...
            if (currentState != newState) {
                BoardEditCommand command = new BoardEditCommand(cursorPosition, this.state.getDrawMode().get(), currentState);
                commandExecutor.execute(command);
            }
